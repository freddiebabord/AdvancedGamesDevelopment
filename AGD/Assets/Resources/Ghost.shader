...
////// Lighting:
////// Emissive:
                float3 node_1105 = (_Colour.rgb*_SoftColour);
                float node_7453 = pow(1.0-max(0,dot(normalDirection, viewDirection)),_EdgeBlend);
                float4 _Texture_var = tex2D(_Texture,TRANSFORM_TEX(i.screenPos.rg, _Texture));
                float3 emissive = (lerp(node_1105,_Colour.rgb,node_7453)*saturate((sceneZ-partZ)/_DepthBelnd)*((lerp(_Colour.rgb,node_1105,_Texture_var.r)*0.2)*node_7453));
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float node_7453 = pow(1.0-max(0,dot(normalDirection, viewDirection)),_EdgeBlend);
                float4 _Texture_var = tex2D(_Texture,TRANSFORM_TEX(i.screenPos.rg, _Texture));
                o.Emission = (lerp(node_1105,_Colour.rgb,node_7453)*saturate((sceneZ-partZ)/_DepthBelnd)*((lerp(_Colour.rgb,node_1105,_Texture_var.r)*0.2)*node_7453));
