...
public class CameraShake : MonoBehaviour
{
    ...
    IEnumerator Shake()
    {
        ...
        while (shakeDuration > 0.01f)
        {
            ...
            if (smooth)
                transform.localRotation = Quaternion.Lerp(transform.localRotation, Quaternion.Euler(rotationAmount),
                    Time.deltaTime*smoothAmount);
            else
                transform.localRotation = Quaternion.Euler(rotationAmount); //Quaternion.Euler(GetComponentInParent<NetworkedThirdPersonCharacter>().m_MouseLook.currentOriginalRoation.eulerAngles + Quaternion.Euler(rotationAmount).eulerAngles);//Set the local rotation the be the rotation amount.
